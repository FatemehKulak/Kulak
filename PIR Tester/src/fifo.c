...
char *_fifo_task_title_[FIFO_MAX_AVAL];

uint8_t FifoCntr = 0;

void fifo_set_front_rear(uint8_t _fifo_id, uint16_t _front, uint16_t _rear) {
	Fifo[_fifo_id].front = _front;
	Fifo[_fifo_id].rear = _rear;
}
...
uint16_t fifo_size(uint8_t _fifo_id) {
	if (Fifo[_fifo_id].rear < Fifo[_fifo_id].front) {
		return (Fifo[_fifo_id].max_elem_aval - Fifo[_fifo_id].front + Fifo[_fifo_id].rear);
	} else {
